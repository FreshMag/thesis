...
errors, and enforcing coding standards. However, such tools are often
unavailable for novel programming paradigms, limiting their adoption and
development. This work explores the application of static analysis in the
context of Aggregate Computing, a paradigm for designing and managing
distributed systems. Specifically, it focuses on \emph{Collektive}, a
Kotlin-based framework that provides an internal \ac{DSL} for Aggregate
Computing.

To improve the reliability of Collektive programs, a compiler plugin was
developed to perform static analysis during compilation. The plugin extends the
Kotlin compiler using the new and experimental \ac{FIR} checkers to detect
potential issues in aggregate computations, particularly regarding \emph{domain
alignment} and misuse of \ac{DSL} constructs. Various detection patterns were
identified and implemented, using techniques ranging from direct API inspections
to tree traversal via the visitor pattern.

The development process, testing methodologies, challenges encountered, and the
trade-offs of integrating static analysis within the compilation pipeline are
discussed. While the plugin provides useful checks and integrates with
development environment, it also highlights the limitations of the approaches
explored, suggesting potential directions for further refinement and
improvements in static analysis for internal DSL-based frameworks.
\end{abstract}
...
require all classes to be \code{open} (i.e., classes that can have subclasses,
the contrary of \code{final} classes, which are default in Kotlin). Instead of
having to manually annotate each class with the \code{open} keyword, this plugin
...
%
In addition, the use of annotations in the code could make it more ``cluttered''
from the perspective of a \ac{DSL}, since it would add elements that strictly
belong to the host language and that clash with a more natural language-like
view. The compiler plugin, instead, would not need these annotations, and could
remain clear and untouched. Of course, Annotation Processors are typically
easier to write and maintain than compiler plugins, but this extra cost can be
worth in several cases, for example in the scenario that will be presented in
this thesis.

\subsection{Main challenges and requirements}

At the time of writing, the development of frontend compiler plugins in Kotlin
is still a relatively less explored area compared to the backend ones. Due to
the limited documentation and examples available, the development of frontend
compiler plugins can be a challenging task, that quite often requires inspecting
the Kotlin compiler source code directly to understand how to interact with it.
%
Frontend compiler plugins can be implemented using \textit{Extensions} to the
Kotlin compiler, a topic which will be explored in more detail in the following
chapters. 
...
As mentioned earlier, the development of the frontend compiler plugin presented
in this thesis will be built on top of an existing backend plugin. This one is
part of a larger project named \emph{Collektive}, a Kotlin multiplatform
framework that provides an \emph{internal} \ac{DSL} for the \textbf{Aggregate
Computing} \cite{DBLP:journals/computer/BealPV15} paradigm. This chapter will
provide an overview of the concepts behind Collektive as well as the features
behind its backend plugin and Kotlin compiler plugins development in general.

\section{Aggregate Computing: a novel paradigm}

The \ac{AC} paradigm is a novel \emph{macro-programming} approach that enables
\textbf{collective behaviors} within a heterogeneous set of devices, inside an
\emph{adaptive} \ac{IoT} system~\cite{DBLP:journals/computer/BealPV15}. The
paradigm shifts the focus from individual devices to regions of devices,
abstracting away the details of their number, position, and behavior. This
abstraction enables developers to reason about distributed systems in terms of
\emph{collective} operations over computational fields, rather than
device-to-device interactions. 

The foundation of Aggregate Programming is built on \textbf{field calculus}
\cite{DBLP:journals/computer/BealPV15}, a set of constructs that enable
manipulations of data structures named \emph{Fields}, which map points in
space-time. These constructs enable the implementation of robust coordination
mechanisms that are \emph{self-stabilizing}, meaning they can adapt to changes
in the environment and input values, gaining scalability to large networks and
preserving resilience
properties~\cite{DBLP:journals/computer/BealPV15}~\cite{DBLP:journals/tomacs/ViroliABDP18}~\cite{DBLP:journals/jlap/ViroliBDACP19}.
Applications of aggregate programming are particularly impactful in large-scale
scenarios, such as crowd management during public events, where distributed
devices coordinate to provide services like crowd density estimation, dispersal
...
\cite{DBLP:conf/ecoop/CasadeiV16} and FCPP~\cite{DBLP:conf/acsos/Audrito20}
extend field calculus principles, providing programming frameworks and language
constructs to bridge the gap between theoretical models and practical
implementations.
...
In Aggregate Computing, the main model of the system consists of a \emph{network
of intercommunicating devices}: each device can be \emph{close} to one another
therefore introducing a concept of \textbf{neighborhood} of devices. 
%
Equipped with \emph{sensors} and \emph{actuators}, they can interact with an
environment and communicate with other devices through a \emph{message-passing}
system. 

A key aspect of \ac{AC} regards the execution model, which is based on a local
program identical for all devices. The system is governed by a continuously
executed loop that makes the devices (1) \emph{receive messages}, (2)
\emph{produce a result through a round of execution of the aggregate program}
and finally (3) \emph{send values to neighbors}. 
%
This structure allows for the system to show collective behaviors emerging from
the network of devices, and contemplates even complex interactions of the
devices with the environment through their sensors and actuators.
...
native platforms.
%
Compared to some other existing \ac{AC} frameworks, Collektive offers a modern
and idiomatic approach to writing aggregate programs, with a static type system
(as it is internal to Kotlin) and few, expressive constructs like
\code{neighboring} and \code{exchange}, which can be used to implement a broad
variety of interactions between devices (and also Aggregate Computing patterns).

The project is organized in modules, with the main one being the \textbf{dsl}
and \textbf{compiler-plugin}, and also provides an integration for
Alchemist~\cite{DBLP:journals/jos/PianiniMV13}, a simulator for pervasive,
aggregate, and nature-inspired computing.
...
Collektive \ac{DSL} is centered around the \code{aggregate} function, which is
the entry point for the aggregate program. The function uses a local ID to
identify the device on which is executed on and then accepts a function as
parameter that performs the aggregate computation using the \code{Aggregate}
interface members.
%
This interface provides several functions that represent the main constructs of
Collektive's implementation of the \ac{AC} paradigm. The most important ones
are:
\begin{itemize}
  \item \code{exchange}: is the base construct that can be used to implement the
  behavior of the other constructs. This operator models an \emph{anisotropic
  communication} with neighbors --- i.e., different information is sent to 
  different neighbors;
  \item \code{share}: models the space-time evolution of the field with an
  \emph{isotropic communication} --- i.e., information is sent uniformly in all
  directions;
  \item \code{neighboring}: observes expressions on neighbors, returning the
  related field;
  \item \code{evolve}: updates an initial value iteratively computing an
  expression at each device.
\end{itemize}

Some of these constructs also have a variant that allows the program to return a
value of a different type than the one \emph{sent} to the neighbors; the name of
the variant is the same of the original function but with the suffix \code{-ing}
(e.g., \code{exchanging}, \code{evolving} etc.). In \cref{lst:dsl-example} is
shown an example of an aggregate program using the Collektive \ac{DSL}: it
starts identifying each device using a \code{localId} and then shares the
maximum value of the field with neighbors starting from an \code{initialValue}.
...
\emph{alignment} inside aggregate programs --- i.e., the alignment of
\emph{devices} that execute aggregate programs --- a very important concept when
dealing with \ac{AC} programs, faced in several studies
...
device computes a value that depends on neighbor devices (e.g., through the
\code{neighboring} construct, which retrieves values from neighbors), those
devices have computed the same expression in the same evaluation round. This
guarantees that shared computations remain consistent across devices: it is
necessary in order to \textbf{maintain consistency between field values} and
\textbf{prevent information leakage}. Without domain alignment, devices may
execute the same function in different rounds, making it impossible to refer 
to the same part of the aggregate program, which is the same for all devices,
and therefore referring to different ``domains'' during the execution.
...
%
As it will be shown in \cref{sec:p2} of the next chapter, this is not always
automatic or safe in certain situations (e.g., loops), and the frontend
extension will take care of these extra checks. 
...
architecture.}.
%
The frontend is independent of the target, and for this reason the result of its
part of the pipeline --- i.e., the frontend \ac{IR} --- can be reused when
targeting different platforms. Starting from the version 2 of the Kotlin
Compiler, the frontend has been upgraded to a new structure called K2, which is
supposed to be much more efficient than the previous K1 version.
%
...
\code{BindingContext}, that holds semantic information such as types and
...
most \emph{internal} \acp{DSL} is the ability to perform static analysis on the
code that is written, and this is particularly difficult since the developers
...
In other words, the frontend plugin can take care of the ``second half of the
problem'' introduced in \cref{sec:dsls}, that is the \emph{static analysis} of
the code that is written in the internal \ac{DSL}, made specifically for
approaching new paradigms with less technical challenges.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \emph{Ktlint}, one of the Kotlin linters available, needs a Gradle plugin to
    be maintained along with the main tool). This means that it can be easily
...
After defining the \code{CollektiveFrontendExtensionRegistrar} class that
...
\code{configurePlugin} method. For this plugin, the extension that will
...
\code{Aggregate} interface as its receiver or inside an \code{aggregate} entry
...
\code{align} \emph{and} \code{dealign} \emph{functions} of the Collektive
...
Given two functions \code{align} and \code{dealign} available in the
\code{Aggregate} interface, the pattern is satisfied when any of these
...
\code{FirExpressionChecker} --- that is, a checker that takes care of
expressions in the code --- typed with a \code{FirFunctionCall} --- i.e., an
element of the \ac{FIR} tree representing a function call in the code. This type
of checker can inspect function calls usages, calling the \code{check} method of
the checker with a \code{FirFunctionCall} as parameter. 
%
...
\lstinputlisting[
  ...
  caption={Implementation of the Pattern 1 checker: \code{ExplicitAlignDealign}}
]{listings/P1Checker.kt}

The \code{context} object is an instance of \code{CheckerContext} and is 
used to provide contextual information to the checker. We will see how to 
use this object in the next sections.

The \code{reportOn} method of the \code{reporter} object is used to report a
...
an object called \code{FirCollektiveErrors}). It is possible to see that the 
...
\lstinputlisting[
  ...
  caption={Some utility functions of the small API developed}
]{listings/SimpleApi.kt}

As shown, we can perform checks on the \ac{FIR} tree using built-in methods like
\code{containingElements}: we can use this method to check if a
\code{FirElement} is contained inside a function that has a \emph{receiver
...
devices) is called inside a loop (e.g. a \code{for} loop), the alignment of
...
operation}, that can be done using the \code{alignedOn} method, another
...
\begin{itemize}
  ...
  \item In the second case, we have the required \code{alignedOn} operation
...
Given a construct that loops through various iterations (e.g., a \code{for}
statement, an anonymous function called when cycling collection's elements, like
\code{map}, \code{forEach} etc.) \emph{inside} an ``aggregate'' block, the
...
\begin{enumerate}
  \item There is no \code{alignedOn} operation that wraps the construct inside
  the loop;
...
\code{alignedOn}, in the correct order. This procedure is summarized in
...
name is one of \code{alignedOn}, \code{align} or \code{dealign}, since these are
...
\lstinputlisting[
  ...
  caption={Implementation of the Pattern 2 checker: \code{NoAlignInsideLoop}}
...
\code{FirElements} (i.e., the nodes of the tree) that are passed to the
checkers.

Kotlin \ac{FIR} visitors are automatically generated inside the Kotlin compiler 
code in order to have a specific method for each type of \code{FirElement}
...
\begin{itemize}
  \item \code{FirVisitor<R, D>}: a visitor where each visit method accepts
a \code{D} parameter that can be used to pass data between the visit calls,
and a return type \code{R} that can be used to return a result from the 
visitor;
  \item \code{FirVisitorVoid}: a visitor where each visit method accepts
only the element to visit and returns nothing. Under the hood is simply a 
\code{FirVisitor<Unit, Nothing?>}.
\end{itemize}
...
\code{yielding} \emph{contexts} in the Collektive \ac{DSL}. 
%
The \code{yielding} context is a specific feature inside the Collektive
\ac{DSL}, briefly introduced in \cref{sec:collektive-dsl}, that allows the
developer to call a construct of the \ac{DSL}, like \code{exchange} for
...
\code{-ing} suffix is used, like \code{exchanging} in this case: these variants
are called \textbf{yielding operations}. 
%
Essentially, a \code{YieldingContext<Initial, Return>} is used inside
yielding operations to act on an \code{Initial} value --- that can, for
example, be exchanged with neighbors --- but \textbf{return a different value of
type} \code{Return} to the caller, without having to return the same value
...
%
In the example provided, the \code{exchanging} construct will still perform like
the normal one, which sends the results from the evaluation of the provided
function to other devices and returns an object of type \code{Field<Int, Int>}.
The entire operation, however, will return another object of type
\code{Field<Int, String>}, resulted from the \code{map} call inside the yielding
action.
...
in a useless \code{yielding} that could be safely substituted with the normal
...
\lstinputlisting[
  ...
  yielding context, this time with the \code{sharing} construct}
]{listings/P3Example.kt}

\subsubsection{Formal description}

Given a Collektive \emph{yielding operation} (e.g., \code{exchanging},
\code{evolving}, etc.), the pattern is satisfied when the expression
returned inside the anonymous function passed as parameter to the
\code{yielding} call is \emph{equivalent} to the one used as its
...
part is the visitor method call \code{containsUnnecessaryYielding},
...
\lstinputlisting[
  ...
  caption={Implementation of the Pattern 3 checker: \code{UnnecessaryYielding}}
...
parameter. When the visitor encounters a function call to the \code{yielding}
operation, it will save the \textbf{explicit receiver} of the call and then
visit the anonymous function that is passed as parameter to \code{yielding}.
%
...
A behavior like this can be implemented using the \code{FirVisitorVoid}
...
The function \code{isStructurallyEquivalentTo} is a utility function that
...
%
The function \code{containsUnnecessaryYielding} is also omitted, but its
...
The anonymous function passed to the \code{map} function is not using the
\code{it} parameter and, in general, this cannot always be interpreted as
...
\code{neighboring} and \code{neighboringViaExchange} constructs are
...
i.e., a return of type \code{Unit}. 
%
...
when the parameters are used \emph{implicitly} (e.g., using the \code{it} keyword).
...
the anonymous function returns \code{Unit} in absence of accepted parameters
(i.e., the \code{neighboring} and \code{neighboringViaExchange} constructs).
...
The two visitors are the \code{ConstructCallVisitor} and
\code{EmptyReturnVisitor}. The latter's implementation is not particularly
...
\code{FirReturnExpression} element, it checks if the return type is
\code{FirUnitExpression}. If so, the pattern is detected.
...
The implementation of the \code{ConstructCallVisitor} is shown in
\cref{lst:p4-visitor}. The variable \code{found} starts off with the value
\code{true} and will be set to false when the visitor encounters a usage of all
...
parameter of type \code{Aggregate} to use as a receiver for the aggregate
constructs (if, instead the delegate would have an explicit receiver of type
\code{Aggregate}, in fact, the pattern would be trivially detected by the
...
\begin{enumerate}
  \item Its signature accepts at least one parameter of type \code{Aggregate};
...
\emph{delegate} nor the functions in the chain contain an \code{alignedOn}
...
parameters is of type \code{Aggregate}. To do this, a new utility function is
implemented: \code{hasAggregateParameter}, shown in \cref{lst:p5-utility}.
%
\begin{lstlisting}[
  float, 
  language=Kotlin,
...
\end{lstlisting}
%
This uses the \code{getArgumentsTypes} and \code{toFqNameUnsafe}
...
to check for the presence of the \code{alignedOn} operation, as its invocation
...
call. Therefore, the presence of \code{alignedOn} will be verified within the
visitor.
...
\end{lstlisting}
%
The \code{@OptIn} is necessary in order to access the \code{fir}
...
a \code{functionCounter} variable is used to keep track of the nesting
...
the Collektive \ac{DSL} that we have seen: \code{evolve}, \code{neighboring} and
\code{share} (and their variants with the yielding context). Since we have not
really explained their behavior in detail, they will be briefly discussed here.

The \code{evolve} construct is used to ``evolve'' the value of a device,
...
starting the evaluation. The \code{neighboring} construct is used to evaluate an
expression in neighbors, constructing a \code{Field} object mapping the
neighbors to the results of the evaluation. The \code{share} construct is used
to compute a \emph{space-time} evolution of the field, computing an expression
over time and sharing it with the neighbors. Since the \code{evolve} construct
too can be used to evolve a field over time, its combination with the
\code{neighboring} construct can behave exactly like a \code{share}, since the
second one can be used to map the \emph{space} aspect. 
%
This is not always true, of course, but using the \code{evolve} construct
could easily lead to this inefficient usage of the DSL constructs,  especially
if the developer is not aware of constructs like \code{share}.

This pattern consists specifically in this type of usage of the \code{evolve}
...
user could first use the \code{neighboring} construct to evaluate the expression
...
anonymous function.
%
Different is the case in which the \code{neighboring} construct is used to
...
These examples of usage and the related version with the \code{share} construct
...
Given an \code{evolve} construct, or its variant, that is passed a function that
contains a \code{neighboring} construct, the pattern is satisfied when the
expression evaluated by \code{neighboring} depends on the value of \code{evolve}
--- i.e., the parameter inside the anonymous function --- and it is used to
compute the value returned by the body of \code{evolve}.
...
\code{evolve} anonymous function --- we need a system to \emph{mark expressions}
...
\code{neighboring} construct, we can immediately know if the expression
...
first extract the receiver of the \code{yielding} construct if we are inside
\code{evolving} and not \code{evolve}, or simply the expression itself if not. 
%
This extra step is necessary for the variants of the construct because the 
value returned by the \code{yielding} is not really relevant to the pattern
...
the one evaluated by the \code{neighboring} construct, and if so the pattern is
detected. 
...
corresponding to a Collektive \ac{DSL} construct, specifically the \code{evolve}
...
visitor encounters a \code{FirResolvedNameReference}, corresponding to a
symbol usage, it will check if the symbol is contained in the list of dependent
symbols. If so, the visitor will set a variable \code{markExpression} to
...
necessary to approach the core of the visitor: the \code{neighboring} and
return expressions. 
...
necessary checks. The \code{neighboring} construct can be used essentially in
two ways:
\begin{enumerate}
...
Once we have saved the expression evaluated by the \code{neighboring} construct,
...
expression of the \code{neighboring} construct if present. If not, the pattern
is skipped. Finally, to compare the return expression with the one we previously
saved, we can use the \code{isStructurallyEquivalentTo} utility function already
...
\code{yielding} construct is not relevant to the pattern detection, and so
...
contained inside the \code{KotlinCompilation.Result} object as a single string
...
\code{TestAggregateInLoop.kt} is a source file contained in the resource
...
functions like \code{forAll} to minimize redundancy.
...
string formatting --- i.e., \code{\%s}. An example is shown in 
...
of writing placeholders with \code{\%s} we can write variables with names 
like \code{\%(nameOfTemplateVariable)} and then pass a configurable map 
through the test code that will replace these variables with the correct values.
All of these utilities, contained in the singleton object \code{CompileUtils},
...
functions, like the \code{alignedOn} seen in \cref{sec:p2} during the
...
\code{content} visible in \cref{lst:poet-extensions} --- to these
...
with other parts, like the \code{alignedOn} construct that wraps the 
...
\code{CollektiveK2JVMCompiler} that automatically compiles a list of source
files returning the diagnostics produced via a \code{MessageCollector} and using
the Collektive compiler plugin. 

In \cref{lst:subjekt-suite} a small example of a reduced Subjekt configuration
used for the tests of Pattern 2 is shown. In this configuration \code{loop},
\code{prettify} and \code{AGGREGATE} are respectively two Macros and one
Parameter that are omitted for brevity. The first one wraps the argument with
several types of iteration constructs, for example a \code{for} loop or a
\code{map} method call on a list. The second is a Macro exposed by Subjekt in
...
the source code (e.g., \code{neighboring}).
