...
this will be called the \emph{delegated function} or simply the \emph{delegate}.
In order to contain aggregate calls, the delegate must have a parameter of type
\lstinline{Aggregate} to use as a receiver for the aggregate constructs (if
instead the delegate would have an explicit receiver of type
...
declared function, here called \emph{delegate}, whose parameters contain one of
type \lstinline{Aggregate} and whose body contains an aggregate construct call,
...
\emph{delegate} nor the functions in the chain contain an \lstinline{alignedOn}
...
condition will perform the same checks as the previous one, but instead of
checking if the function call being examined is an aggregate one, it will check
if one of its parameters are of type \lstinline{Aggregate}. To do this a new
utility function is implemented: \lstinline{hasAggregateParameter}, shown in
\cref{lst:p5-utility}.
...
brevity). Once this condition is satisfied, the visitor will be called to
...
need to take \emph{multiple visiting call} of the same method and even
\emph{recursive visiting} with another visitor into consideration. Finally,
we will approach the pattern that was considered the most complex to detect,
where multiple types of visitors were used together to inspect the code.
